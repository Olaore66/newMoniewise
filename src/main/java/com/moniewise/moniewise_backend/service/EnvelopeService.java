...
//    }
    ...
    private LocalDateTime fetchCurrentDateTimeFromDatabase() {
        return java.time.ZonedDateTime.now(java.time.ZoneId.of("Africa/Lagos")).toLocalDateTime();
    }

    private LocalDateTime fetchUserLocalTime(User user) {
        // 1. Get UTC Time (The Absolute Truth)
        ZonedDateTime nowUTC = ZonedDateTime.now(ZoneId.of("UTC"));

        // 2. Convert to User's Timezone (The Relative Truth)
